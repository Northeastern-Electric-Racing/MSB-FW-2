...
VL53L7CX_RANGING_SENSOR_Drv_t VL53L7CX_RANGING_SENSOR_Driver = {
    VL53L7CX_Init,          VL53L7CX_DeInit,
    VL53L7CX_ReadID,        VL53L7CX_GetCapabilities,
    VL53L7CX_ConfigProfile, VL53L7CX_ConfigROI,
    VL53L7CX_ConfigIT,      VL53L7CX_GetDistance,
    VL53L7CX_Start,         VL53L7CX_Stop,
    VL53L7CX_SetAddress,    VL53L7CX_GetAddress,
    VL53L7CX_SetPowerMode,  VL53L7CX_GetPowerMode};
 ...
 * @{
 */
static int32_t vl53l7cx_poll_for_measurement(VL53L7CX_Object_t *pObj,
                                             uint32_t Timeout);
static int32_t vl53l7cx_get_result(VL53L7CX_Object_t *pObj,
...
int32_t VL53L7CX_ConfigProfile(VL53L7CX_Object_t *pObj,
  ...
  if (ret != VL53L7CX_OK) {
    return ret;
  } else if (vl53l7cx_set_resolution(&pObj->Dev, resolution) !=
             VL53L7CX_STATUS_OK) {
    ret = VL53L7CX_ERROR;
  } else if (vl53l7cx_set_ranging_mode(&pObj->Dev, ranging_mode) !=
             VL53L7CX_STATUS_OK) {
    ret = VL53L7CX_ERROR;
  } else if (vl53l7cx_set_integration_time_ms(&pObj->Dev, integration_time) !=
             VL53L7CX_STATUS_OK) {
    ret = VL53L7CX_ERROR;
  } else if (vl53l7cx_set_ranging_frequency_hz(&pObj->Dev, ranging_frequency) !=
             VL53L7CX_STATUS_OK) {
    ret = VL53L7CX_ERROR;
  } else {
...
int32_t VL53L7CX_SetAddress(VL53L7CX_Object_t *pObj, uint32_t Address) {
  ...
  if (pObj == NULL) {
    ret = VL53L7CX_INVALID_PARAM;
  } else if (vl53l7cx_set_i2c_address(&pObj->Dev, (uint8_t)Address) !=
             VL53L7CX_STATUS_OK) {
    ret = VL53L7CX_ERROR;
  } else {
...
int32_t VL53L7CX_SetPowerMode(VL53L7CX_Object_t *pObj, uint32_t PowerMode) {
  ...
  if (pObj == NULL) {
    ...
    ret = VL53L7CX_INVALID_PARAM;
  } else if (vl53l7cx_set_power_mode(&pObj->Dev, (uint8_t)PowerMode) !=
             VL53L7CX_STATUS_OK) {
    ret = VL53L7CX_ERROR;
  } else {
    ret = VL53L7CX_OK;
...
int32_t VL53L7CX_GetPowerMode(VL53L7CX_Object_t *pObj, uint32_t *pPowerMode) {
  ...
  if ((pObj == NULL) || (pPowerMode == NULL)) {
    ret = VL53L7CX_INVALID_PARAM;
  } else if (vl53l7cx_get_power_mode(&pObj->Dev, &powermode) !=
             VL53L7CX_STATUS_OK) {
    *pPowerMode = 0;
    ret = VL53L7CX_ERROR;
  } else {
...
int32_t VL53L7CX_XTalkCalibration(VL53L7CX_Object_t *pObj, uint16_t Reflectance,
  ...
  status =
      vl53l7cx_calibrate_xtalk(&pObj->Dev, Reflectance, nb_samples, Distance);
...
static int32_t vl53l7cx_get_result(VL53L7CX_Object_t *pObj,
  ...
  if ((pObj == NULL) || (pResult == NULL)) {
    ret = VL53L7CX_INVALID_PARAM;
  } else if (vl53l7cx_get_resolution(&pObj->Dev, &resolution) !=
             VL53L7CX_STATUS_OK) {
    ret = VL53L7CX_ERROR;
  } else if (vl53l7cx_get_ranging_data(&pObj->Dev, &data) !=
             VL53L7CX_STATUS_OK) {
    ret = VL53L7CX_ERROR;
  } else {
    ...
    for (i = 0; i < resolution; i++) {
      ...
      for (j = 0; j < data.nb_target_detected[i]; j++) {
        pResult->ZoneResult[i].Distance[j] =
            (uint32_t)data.distance_mm[(VL53L7CX_NB_TARGET_PER_ZONE * i) + j];
        ...
        if (pObj->IsSignalEnabled == 1U) {
          pResult->ZoneResult[i].Signal[j] =
              (float_t)
                  data.signal_per_spad[(VL53L7CX_NB_TARGET_PER_ZONE * i) + j];
        } else {
          pResult->ZoneResult[i].Signal[j] = 0.0f;
        }

        target_status =
            data.target_status[(VL53L7CX_NB_TARGET_PER_ZONE * i) + j];
        pResult->ZoneResult[i].Status[j] =
            vl53l7cx_map_target_status(target_status);
      }
    }
