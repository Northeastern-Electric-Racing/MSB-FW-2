...
void _delay(uint32_t delay) { tx_thread_sleep(delay); }
...
uint16_t imu_get_accelerometer_data(LSM6DSV_Axes_t *axes) {
    ...
    if (status != 0) {
        PRINTLN_INFO(
            "ERROR: Failed to call lsm6dsv_acceleration_raw_get() (Status: %d).",
            status);
        return U_ERROR;
    }
...
uint16_t imu_get_gyroscope_data(LSM6DSV_Axes_t *axes) {
    ...
    if (status != 0) {
        PRINTLN_INFO(
            "ERROR: Failed to call lsm6dso_angular_rate_raw_get() (Status: %d).",
            status);
        return U_ERROR;
    }
...
uint16_t init_imu() {
    ...
    if (status != 0) {
        PRINTLN_ERROR("Failed to call lsm6dsv_device_id_get() (Status: %d).",
                status);
        return U_ERROR;
    }
    ...
    _delay(30); // This is probably overkill, but the datasheet lists the
                // gyroscope's "Turn-on time" as 30ms, and I can't find anything
                // else that specifies how long resets take.

    /* Enable Block Data Update. */
    status = lsm6dsv_block_data_update_set(
        &imu, PROPERTY_ENABLE); // Makes it so "output registers are not updated
                                // until LSB and MSB have been read". Datasheet
                                // says this is enabled by default but figured it
                                // was better to be explicit.
    if (status != 0) {
...
int32_t _lis2mdl_read(void *handle, uint8_t register_address, uint8_t *data,
                      uint16_t length) {
    uint8_t spi_reg = (uint8_t)(register_address | 0x80);
    HAL_StatusTypeDef status;
...
void send_hdc2021_data() {
    can_msg_t can_message = { 
        .id = convert_can_id(HDC2021_CAN_ID), 
        .len = 8, 
        .data = { 0 } 
    };                
